...
class Algorithms {
    ...
    fun trig1(year: Int, month: Int, day: Int): Double {
        ...
        val M0 = 360 * (K0 * 0.08084821133)%1 + 359.2242 - 0.0000333 * T2 - 0.00000347 * T3
        val M1 = 360 * (K0 * 0.07171366128)%1 + 306.0253 + 0.0107306 * T2 + 0.00001236 * T3
        val B1 = 360 * (K0 * 0.08519585128)%1 + 21.2964 - 0.0016528 * T2 - 0.00000239 * T3
        var oldJ = 0.0
        ...
        while (jday < thisJD) {
            ...
            F += 0.0161 * sin(2 * M6.toDouble()) + 0.0104 * sin(2 * B6.toDouble())
            F -= 0.0074 * sin(M5 - M6.toDouble()) - 0.0051 * sin(M5 + M6.toDouble())
            F += 0.0021 * sin(2 * M5.toDouble()) + 0.0010 * sin(2 * B6 - M6.toDouble())
            F += 0.5 / 1440
            oldJ = jday
            jday = J0 + 28.0 * phase + floor(F)
            phase++
        }
